...
def compute_embeddings(args: argparse.Namespace) -> None:
    ...
    if args.backend == "transformers":
        ...
        all_caption_embeddings_vllm: list[torch.Tensor] = []
        all_image_embeddings_vllm: list[torch.Tensor] = []
        ...
        for i in range(0, len(caption_inputs), args.batch_size_text):
            ...
            all_caption_embeddings_vllm.append(batch_caption_embeddings)

        caption_embeddings = torch.cat(all_caption_embeddings_vllm, dim=0)
        ...
        for i in range(0, len(image_inputs), args.batch_size_image):
            ...
            all_image_embeddings_vllm.append(batch_image_embeddings)

        image_embeddings = torch.cat(all_image_embeddings_vllm, dim=0)
        image_embeddings = image_embeddings.repeat_interleave(
            torch.tensor(caption_counts, device=image_embeddings.device), dim=0
        )
